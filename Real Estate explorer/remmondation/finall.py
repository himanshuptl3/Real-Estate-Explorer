...
def get_inputs_and_recommend(city, rec_type):
    ...
    if city == "Gurgaon":
        ...
        if rec_type == "Location Based":
            ...
            st.subheader("📊 Predict House Price (Bangalore)")
            ...
            if st.button("Predict Price"):
                try:
                    ...
                    st.error("Model file not found. Ensure 'bangalore_rf_model.pkl' exists.")
        ...
        if rec_type == "Location Based":
            ...
            st.subheader("📊 Predict House Price (Pune)")
            total_sqft = st.number_input("Enter total_sqft", value=1000)
            bath = st.number_input("Enter number of bathrooms", value=2)
            balcony = st.number_input("Enter number of balconies", value=1)
            ...
            if st.button("Predict Price"):
                try:
                    model = joblib.load('pune_rf_model.pkl')
                    input_data = [[total_sqft, bath, balcony]]
                    predicted_price = model.predict(input_data)[0]
                    st.success(f"🏷️ Estimated Price: ₹ {round(predicted_price, 2)} Lakhs")
                except FileNotFoundError:
                    st.error("Model file not found. Ensure 'pune_rf_model.pkl' exists.")
        ...
        if rec_type == "Location Based":
            ...
            st.subheader("📊 Predict House Price (Delhi)")
            area = st.number_input("Enter area (sqft)", value=1000)
            bhk = st.number_input("Enter number of BHKs", value=2)
            bathroom = st.number_input("Enter number of bathrooms", value=2)

            if st.button("Predict Price"):
                try:
                    model = joblib.load('delhi_rf_model.pkl')
                    input_data = [[area, bhk, bathroom]]
                    predicted_price = model.predict(input_data)[0]
                    st.success(f"🏷️ Estimated Price: ₹ {round(predicted_price, 2)} Lakhs")
                except FileNotFoundError:
                    st.error("Model file not found. Ensure 'delhi_rf_model.pkl' exists.")
